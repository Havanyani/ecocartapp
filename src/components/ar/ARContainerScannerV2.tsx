...
interface ARContainerScannerProps {
  ...
  testMode?: boolean; // For testing without camera
}

/**
 * Unified component for scanning and recognizing recycling containers using AR
 * with material detection capabilities
 */
export default function ARContainerScannerV2({
  ...
  // Handle camera permissions (either real or simulated for tests)
  useEffect(() => {
    const requestPermissions = async () => {
      if (testMode) {
        // In test mode, simulate permission granted
        setTimeout(() => setHasPermission(true), 500);
      } else {
        try {
          // In real app, we would use expo-camera here
          // For now, just simulate permission granted
          setTimeout(() => setHasPermission(true), 500);
          
          // When integrating real camera:
  ...
  // Simulate container recognition
          ...
          if (recognitionResult.container) {
            try {
              ...
              const updatedContainer = {
                ...
                environmentalImpact: {
                  ...recognitionResult.container.environmentalImpact,
                  // Add calculated impact based on material type
                  landfillSpaceSaved: calculateLandfillSpaceSaved(materialResult.materialType as CommonMaterials)
                }
              };
  ...
  // Calculate landfill space saved based on material type
  const calculateLandfillSpaceSaved = (materialType: CommonMaterials): number => {
    // Values in cubic centimeters
    switch (materialType) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Simulated Camera View - In a real app, this would be the Camera component */}
