...
export default function TwoFactorSetupScreen() {
  ...
  useEffect(() => {
    async function generateSecret() {
      if (!isOnline) {
        setFormErrors(prev => ({
          ...prev,
          general: 'You need to be online to set up two-factor authentication'
        }));
      ...
      try {
        ...
        setFormErrors(prev => ({
          ...prev,
          general: 'Failed to generate secret. Please try again later.'
        }));
  ...
  const handleCodeChange = (index: number, value: string) => {
    ...
    newCode[index] = value;
    setFormData(prev => ({ ...prev, verificationCode: newCode.join('') }));
  ...
  const handleVerify = async () => {
    ...
    if (formData.verificationCode.length !== 6) {
      setFormErrors(prev => ({
        ...prev,
        verificationCode: 'Please enter a 6-digit verification code'
      }));
      return;
    }
    ...
    if (!secret) {
      setFormErrors(prev => ({
        ...prev,
        general: 'Secret not generated. Please try again.'
      }));
      return;
    }
